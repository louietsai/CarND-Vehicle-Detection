...
def find_cars(img, ystart, ystop, xstart, xstop, scale, step):
    ...
    #print("find_cars img_to_search x,y ",xstart,xstop,ystart,ystop)
    ystart=int(ystart)
    ystop=int(ystop)
    xstart=int(xstart)
    xstop=int(xstop)
...
def draw_labeled_bboxes(labels):
    global track_list
    #print("draw_labeled_bboxes len of tack_list : ",len(track_list))
    track_list_l = []
    ...
    #print("draw_labeled_bboxes after track_list_l len of tack_list : ",len(track_list))
    boxes = []
...
def frame_proc(img, lane = False, video = False, vis = False):
    ...
    if (video and n_count%2==0) or not video: # Skip every second video frame
        ...
        #print("new scan boxes len:",len(boxes))
        if track_list == []:
            #print("empty track list")
            boxes += find_cars(img, 400, 700, 0, 1280, 0.75, 3)
        updated_track_list = []
        for track in track_list:
            track_boxes = []
            ...
            track_boxes+=find_cars(img, ys, yf, xs, xf, scale, 2)
            track_boxes+=find_cars(img, ys, yf, xs, xf, scale*1.25, 2)
            track_boxes+=find_cars(img, ys, yf, xs, xf, scale*1.5, 2)
            track_boxes+=find_cars(img, ys, yf, xs, xf, scale*1.75, 2)
            #print("track_boxes ,boxes len",len(track_boxes),len(boxes))
            if track_boxes != []:
        ...
        if len(track_list) != len(updated_track_list):
            #print("track_list != updated_track_list",len(track_list),len(updated_track_list))
        ...
        cars_boxes = boxes_p
    print("cars_boxes boxes ",len(cars_boxes),len(boxes))
...
print("car sample size : ",len(cars))
print("not car sample size : ",len(notcars))
...
for image_p in glob.glob('test_images/test*.jpg'):
    ...
    print("Start Single Sliding Windows with search",image_p)
    show_img(window_img)
    print("End Single Sliding Windows with search")
...
print("Start multiple sliding windows")
show_img(window_img)
print("End multiple sliding windows")
...
windows = slide_window(image, x_start_stop=[track[0]-w_size,track[0]+w_size],
                       ...
                       xy_window=(48, 48), xy_overlap=(0.75, 0.75))
window_img = draw_boxes(window_img, windows, color=(255, 0, 0), thick=6)
print("Start multiple sliding windows 2")
show_img(window_img)
print("End multiple sliding windows 2")
...
for image_p in glob.glob('test_images/test*.jpg'):
    print("frame_proc image : ", image_p)
    image = cv2.imread(image_p)
    proc_img = frame_proc(image, lane=True, vis=False)
    print("Start frame proc")
    show_img(proc_img)
    print("End frame proc")
...
while cap.isOpened():
    ...
    #    show_img(proc_img)
    #print("End frame proc")
    #outfilepath = "proc_images/"+"file_"+str(i)+".png"
    #print("outfilepath : ", outfilepath)
    #cv2.imwrite(outfilepath,proc_img)
    cv2.imshow('window-name',proc_img)
    cv2.waitKey(1)
    out.write(proc_img)
cap.release()
out.release()
